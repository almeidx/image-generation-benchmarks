...
async function init() {
  ...
  $("scenario-select").onchange = renderBench;
  $("format-select").onchange = renderBench;

  for (const r of data.runs) $("coldstart-runtime").append(new Option(runtimeLabel(r), runtimeLabel(r)));
  $("coldstart-runtime").onchange = renderColdStart;
...
function benchRows(scenario, format) {
  ...
  return adapters
    .map((adapter) => ({
      adapter,
      cells: data.runs.map(
        (r) => r.benchmarks.find((b) => b.adapter === adapter && b.scenario === scenario && b.format === format),
      ),
    }))
...
function renderBench() {
  ...
  benchChart = new Chart($("bench-chart"), {
    ...
    options: {
      indexAxis: "y",
      scales: {
        x: { title: { display: true, text: "mean ms (log scale)" }, type: "logarithmic", ticks: { color: "#94a3b8" }, grid: { color: "#334155" } },
  ...
  $("bench-table").innerHTML = `
    <table>
      ...
      <tbody>${rows
        .map(
          (row) => `<tr><td>${row.adapter}</td>${row.cells
            .map((cell, i) =>
              cell
...
function renderColdStart() {
  ...
  coldChart = new Chart($("coldstart-chart"), {
    ...
    options: {
      indexAxis: "y",
      scales: {
        x: { stacked: true, title: { display: true, text: "ms" }, ticks: { color: "#94a3b8" }, grid: { color: "#334155" } },
...
function renderSimilarity() {
  ...
  $("similarity-table").innerHTML = scenarios
    .map((scenario) => {
      const pairs = v.crossLibrary.filter((c) => c.scenario === scenario).sort((a, b) => b.diffRatio - a.diffRatio);
      return `
