...
if (!base) {
	...
	const flagged = rows
		.filter((r) => Math.abs(r.delta) >= FLAG_THRESHOLD)
	...
	if (added.length > 0) {
		lines.push(
			`**Newly supported:** ${added.map((b) => `${b.adapter} · ${b.scenario} · ${b.format}`).join(", ")}`,
		);
		lines.push("");
	}
	if (removed.length > 0) {
		lines.push(
			`**No longer supported:** ${removed.map((b) => `${b.adapter} · ${b.scenario} · ${b.format}`).join(", ")}`,
		);
		lines.push("");
	}
