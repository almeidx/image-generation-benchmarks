...
for (const combo of prepared.combos) {
  ...
  if ("error" in result) {
    baselines.push({ adapter: combo.adapter.name, scenario: combo.scenario.name, diffRatio: 1, pass: false });
  ...
  console.log(`${pass ? "ok  " : "FAIL"} ${combo.adapter.name}/${combo.scenario.name}: ${(result.ratio * 100).toFixed(3)}% differs`);
}
...
if (missingBaselines.length > 0) {
  console.error(
    `missing baselines (run: npm run baselines:update): ${missingBaselines
