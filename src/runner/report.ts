...
lines.push("");
lines.push("Mean time per render+encode (operations/second in parentheses). Lower is better; the fastest library per column is bold.");
lines.push("");
for (const scenario of allScenarios) {
  ...
  for (const format of allFormats) {
    const rows = allAdapters
      .map((adapter) => ({
        adapter,
        cells: benchFiles.map(
          (f) => f.benchmarks.find((b) => b.adapter === adapter && b.scenario === scenario && b.format === format),
        ),
      }))
...
if (validationFiles.length > 0) {
  ...
  const failures = validationFiles.flatMap((v) =>
    v.baselines.filter((b) => !b.pass).map((b) => `| ${b.adapter} | ${runtimeLabel(v)} | ${b.scenario} | ${(b.diffRatio * 100).toFixed(2)}% |`),
  );
  ...
  for (const scenario of scenariosInV) {
    ...
    lines.push("| --- | --- |");
    for (const p of pairs.sort((a, b) => b.diffRatio - a.diffRatio)) {
...
lines.push("---");
lines.push("");
lines.push("Methodology: see the repository README. Setup (fonts, image decoding) is excluded from steady-state numbers; every library renders identical scenario inputs and encodes with identical quality settings.");
lines.push("");
...
const galleryRuntime = runtimes.filter((r) => r.startsWith("node")).sort().pop() ?? runtimes[0];
if (galleryRuntime) {
